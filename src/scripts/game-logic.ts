...
export const guessChecker = (userInput: number[], secretNumber: number[]): GuessResult => {
    let bulls = 0;
    let cows = 0;
    ...
    //console.log(`Correct Position: ${bulls}, Wrong Position: ${cows}, Message: ${message}, Fullfill Win Condition?: ${isWin}`)
...
export const validateInput = (input: string): ValidationResult => {
    ...
    if (digits.includes(0)) {
        return { isValid: false, error: 'Zero is not allowed. Use digits 1-9 only', digits: [] };
    }
